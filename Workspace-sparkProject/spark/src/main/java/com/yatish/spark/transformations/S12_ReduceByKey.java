	...
	public static void main(String[] args) {
		...
		JavaPairRDD<Integer,Integer> rddData3 = rddData1.reduceByKey(new Function2<Integer,Integer,Integer>() {
			...
			public Integer call(Integer a,Integer b) {
				System.out.println("reduceByKey called");
				return a+b;
			}
		});
		...
		/*
		 * There is one more variation of '.reduceByKey' which is '.reduceByKey(Function..., int numOfPartitions)'. it will partitions the transformed RDD based on the input agrument 'numOfPartitions'
		 * in this case, we have given '5'
		 */
		JavaPairRDD<Integer,Integer> rddData4 = rddData1.reduceByKey((valueOfPair1,valueOfPair2) -> valueOfPair1+valueOfPair2, 5);
		
		
		/*
		 * There is one more variation of '.reduceByKey' which is '.reduceByKey(Paritioner, Function2)'.
		 * 
		 * If user wants to use his own custom partitioner for partitioning instead of default partitioner. Then it will be very useful.
		 */
		Custom_Partitioner obj = new Custom_Partitioner(3);
		JavaPairRDD<Integer,Integer> rddData5 = rddData1.reduceByKey(obj,new Function2<Integer,Integer,Integer>() {
		...
		/*
		 * 
		 * NOTE: This will return 'Map' instead of 'JavaPairRDD'. This will behave like combination of 'collect()' and 'reduceByKey' as this will do reduceByKey operation and will return all the RDDs
		 * 			to the master node in form of 'Map'
		 */
		Map<Integer,Integer> rddData6 = rddData1.reduceByKeyLocally(new Function2<Integer,Integer,Integer>() {
			
			public Integer call(Integer a,Integer b) {
				System.out.println("reduceByKeyLocally called");
				return a+b;
			}
		});
